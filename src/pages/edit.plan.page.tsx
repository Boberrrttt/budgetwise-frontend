...
const EditPlanPage = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      setLoading(true);
      const fetchedItems = await axiosInstance.get(`/api/getItems?planId=${budgetPlan.id}`)
      ...
      <div className="flex w-full h-full">
        <div className="flex flex-col flex-grow items-center mt-10">
          ...
          <div className="flex w-full items-center flex-col mt-20">
            <div className="flex w-full justify-center items-center gap-[25rem]">
              {loading ? (
                  <CircularProgress/>
                ): (
                  items.map( (item, index) => (
                    <div className="w-full flex flex-col items-center" key={index}>
                      {item.name !== 'plus-button'
                        ? (
                          <>
                            <h1 className="text-2xl">Pencils</h1>
                            <div className="flex">
                                <svg className=" mr-3" xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24">
                                ...
                                <h1 className="text-2xl font-bold">P 50</h1>
                            </div>
                          </>
                        ) : (
                          <>
                      ...
                      <div className="w-[70%] bg-blue-600 h-1 rounded-full mt-24"></div>          
                    </div>
                  ))
                )
              }
        ...
        {isPopup && <AddItemPopup isPopup={isPopup} setIsPopup={setIsPopup} />}
      </div>
    </div>
  );
};
